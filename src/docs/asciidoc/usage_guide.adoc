...
If you're coming to Javaslang after using Java's `Optional` class, there is a crucial difference.  In `Optional`, a call to `.map` that results in a null will result in an empty `Optional`.  In Javaslang, it would result in a `Some(null)` that can then lead to a `NullPointerException`.

Using `Optional`, this scenario is valid.

[source,java,indent=0]
----
include::../../test/java/javaslang/OptionDemo.java[tags=javaOptionalWithMappedNull]
----
<1> The option is `Some("foo")`
<2> The resulting option becomes empty here

Using Javaslang's `Option`, the same scenario will result in a `NullPointerException`.

[source,java,indent=0]
----
include::../../test/java/javaslang/OptionDemo.java[tags=javaslangOptionWithMappedNull]
----
<1> The option is `Some("foo")`
<2> The resulting option is `Some(null)`
<3> The call to `s.toUpperCase()` is invoked on a `null`

This seems like Javaslang's implementation is broken, but in fact it's not - rather, it adheres to the requirement of a monad to maintain computational context when calling `.map`.  In terms of an `Option`, this means that calling `.map` on a `Some` will result in a `Some`, and calling `.map` on a `None` will result in a `None`.  In the Java `Optional` example above, that context changed from a `Some` to a `None`.
...
[source,java,indent=0]
----
include::../../test/java/javaslang/OptionDemo.java[tags=flatMapNullParameter]
----
<1> The option is `Some("foo")`
<2> The resulting option is `Some(null)`
...
[source,java,indent=0]
----
include::../../test/java/javaslang/OptionDemo.java[tags=mapOptionParameter]
----
<1> The option is `Some("foo")`
<2> The resulting option is `None`
<3> This step never occurs because the option is a `None`

This is explored in more detail on the http://blog.javaslang.io/the-agonizing-death-of-an-astronaut/[Javaslang blog].
...
[source,java,indent=0]
----
include::../../test/java/javaslang/LazyDemo.java[tags=createLazy]
----
...
| LinkedHashMap | const^eff^   | const^eff^ | linear     | linear
| LinkedHashSet | const^eff^   | const^eff^ | linear     | linear
...
* const^a^ &mdash; amotized constant time, few operations may take longer
