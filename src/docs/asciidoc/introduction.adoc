...
image::images/javaslang-collections.png?w=660[Javaslang Collections]
...
----
// = List(0, 2, 3)
List list2 = list1.tail().prepend(0);
...
----
// = TreeSet(1, 2, 3);
SortedSet set = TreeSet.of(2, 3, 1, 2);
...
Lambda brought Java and Scala closer together, yet they are still so different. Martin Odersky, the creator of Scala, recently mentioned in hishttps://www.youtube.com/watch?v=NW5h8d_ZyOs[BDSBTB 2015 keynote] the state of the Java 8 collections.
